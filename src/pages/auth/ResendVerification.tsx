...
const parseApiError = (
  ...
  if (typeof detail === "object" && detail !== null) {
    return {
      message: detail.message || "FAILED TO RESEND VERIFICATION LINK",
...
export default function ResendVerificationPage() {
  ...
  // Dynamically replace [REMAINING] or [remaining] placeholders with the live countdown value
  const dynamicErrorMessage = errorMessage
    ? errorMessage.replace(/\[REMAINING\]/gi, `${cooldown}`)
    : null;

  useEffect(() => {
    if (cooldown <= 0) {
      if (errorMessage && errorMessage.includes("COOLDOWN")) {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (cooldown > 0) {
      setErrorMessage(
        `A COOLDOWN IS ACTIVE, PLEASE WAIT ANOTHER [REMAINING] SECONDS BEFORE REQUESTING A NEW VERIFICATION LINK.`
      );
      return;
    }
    ...
    try {
      ...
      const res = await resendVerifyApi({ email: email.trim() });
      const displayTitle = res?.title?.toUpperCase() || "SUCCESS";
      const displayMsg = (
        res?.message || "VERIFICATION LINK SENT SUCCESSFULLY"
      ).toUpperCase();
        ...
        >
          ...
          >
            <div className="overflow-hidden">
              {dynamicErrorMessage && (
                <div
                  key={errorMessage} // Stable Key stops re-triggering entrance animation on tick
