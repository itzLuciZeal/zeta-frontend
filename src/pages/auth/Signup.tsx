...
import { sanitizeUsername, isValidUsername } from "../../utils/username";
...
export default function SignupPage() {
  ...
  const isUsernameValid = username.length >= 3 && isValidUsername(username) && !usernameError;
  const isEmailValid = email.length > 0 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) && !emailError;
  const isPasswordValid = password.length >= 8 && password.length <= 72 && !passwordError;
  const isConfirmValid = confirmPassword.length > 0 && confirmPassword === password && !confirmError;
  ...
  const handleUsernameBlur = () => {
    if (!username) {
      ...
      setUsernameError("ALPHANUMERIC ONLY, 3-30 CHARACTERS");
    } else {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (!isValidUsername(username)) {
      setErrorMessage("USERNAME MUST BE ALPHANUMERIC (3-30 CHARACTERS)");
      return;
    }
  ...
  return (
    <div className="relative flex flex-col items-center justify-start sm:justify-center h-dvh w-full select-none px-3 sm:px-4 py-8 sm:py-12 overflow-x-hidden overflow-y-auto">
      ...
      {isSubmitting && <PersonaLoading message="CREATING ACCOUNT..." />}
        ...
        >
          ...
          <div className="text-center pt-3 border-t-2 border-cyan-900/60 flex justify-center">
            ...
            >
              ...
              <span className="text-[9px] sm:text-[10px] font-black italic text-cyan-400 group-hover:text-[#020612] group-focus-visible:text-[#020612] transition-colors duration-200">
              </span>
            </Link>
          </div>
        </form>
